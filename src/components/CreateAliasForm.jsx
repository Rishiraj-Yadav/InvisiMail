...
export default function CreateAliasForm({
  ...
  return (
    <div className="surface-card rounded-2xl border border-border shadow-xl">
      <div className="px-6 py-5 border-b border-border">
        <div className="mt-5 p-4 surface-interactive rounded-xl border border-border">
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${
              isPro ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-muted text-[hsl(var(--muted-foreground))] border-border'
            }`}>
          ...
          {!isPro && (
            <div className="mt-3 pt-3 border-t border-border">
              <div className="flex items-center justify-between text-sm mb-1.5">
                <span className="text-[hsl(var(--muted-foreground))]">Personal aliases used:</span>
              ...
              <div className="w-full bg-muted rounded-full h-2 overflow-hidden">
                <div 
                  className={`h-full rounded-full transition-all duration-300 ${
                    personalAliasesCount >= 5 ? 'bg-red-400' : 'bg-card'
                  }`}
      ...
      <div className="p-6">
        <form onSubmit={(e) => handleCreateAlias(e, selectedDomain)} className="space-y-6">
          <div>
            ...
            <div className="flex items-center w-full h-12 bg-card/70 border border-border rounded-xl focus-within:border-[#6366F1] focus-within:ring-2 focus-within:ring-[#6366F1]/20 transition-all overflow-hidden">
              <input
                id="alias-input"
                type="text"
                placeholder="e.g., support, contact, info"
                className={`flex-1 h-full px-4 bg-transparent text-sm font-medium text-foreground placeholder-white/40 focus:outline-none ${
              ...
              />
              <span className="inline-flex items-center px-4 h-full text-[hsl(var(--muted-foreground))] bg-muted border-l border-border text-sm font-medium">
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all duration-200 ${
                canCreateMore && newAlias.trim() && !submitting
                  ? 'bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] text-foreground hover:from-[#4F46E5] hover:to-[#7C3AED] shadow-lg shadow-indigo-500/25 cursor-pointer'
                  : 'bg-muted text-foreground/30 border border-border cursor-not-allowed'
              }`}
            >
              {submitting ? (
                <>
                  <div className="animate-spin rounded-full h-4 w-4 border-2 border-border border-t-white"></div>
                  Creating...
                </>
              ) : (
                <>
        ...
        {!isPro && (
          <div className="mt-8 p-5 border border-border rounded-xl relative overflow-hidden">
            ...
            <div className="flex items-start gap-4 relative z-10">
              ...
              <div>
                ...
                <ul className="text-xs text-[hsl(var(--muted-foreground))] mt-2 space-y-1.5">
                  <li className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-card"></span> Unlimited email aliases</li>
                  <li className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-card"></span> Collaborative team aliases</li>
                  <li className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-card"></span> Custom domains</li>
                  <li className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-card"></span> Advanced analytics</li>
                  <li className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-card"></span> Priority support</li>
                </ul>
